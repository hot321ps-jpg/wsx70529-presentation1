...
export default function Page() {
  ...
  const tickerText = useMemo(() => {
    ...
    for (const e of latestEvents.slice(0, 3)) pieces.push(`${e.level.toUpperCase()}: ${e.title}`);
    return pieces.join("  •  ");
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-neutral-950 text-neutral-100">
      ...
      <div className="pointer-events-none absolute inset-0">
        <div className={`absolute -top-40 left-1/2 h-[520px] w-[860px] -translate-x-1/2 rounded-full bg-gradient-to-r ${liveAccent} blur-3xl`} />
        ...
        <div
          ...
          style={{
            backgroundImage: "linear-gradient(to bottom, rgba(255,255,255,0.9) 1px, transparent 1px)",
      ...
      <div className="relative mx-auto max-w-7xl px-5 py-10">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className={`h-12 w-12 rounded-2xl bg-gradient-to-br from-cyan-400/30 via-fuchsia-500/20 to-emerald-400/25 ${liveGlow} border border-white/10`} />
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs ${pillTone(status)}`}>
                  <span className={`h-1.5 w-1.5 rounded-full ${
                    status === "live" ? "bg-emerald-300" : status === "degraded" ? "bg-amber-300" : status === "error" ? "bg-red-300" : "bg-neutral-300"
                  }`} />
                  {pillLabel(status)}
                </span>
              </div>
              <p className="mt-1 text-sm text-neutral-400">Twitch 真資料 · KV 快取 · 去重鎖 · SSE 推送 · Hobby 免 Cron</p>
            </div>
          </div>
        ...
        <div className={`mt-6 rounded-2xl border border-white/10 ${isLive ? "bg-emerald-400/10" : "bg-white/5"} p-4`}>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <span className={`h-2.5 w-2.5 rounded-full ${isLive ? "bg-emerald-300 shadow-[0_0_18px_rgba(52,211,153,0.75)]" : "bg-neutral-400"}`} />
          ...
          <div className="lg:col-span-8">
            <div className="grid gap-4 md:grid-cols-3">
              <div className={`rounded-2xl border border-white/10 bg-white/5 p-5 ${liveGlow}`}>
                <div className="flex items-center justify-between">
                  ...
                  <div className={`text-[11px] ${isLive ? "text-emerald-200" : "text-neutral-400"}`}>{isLive ? "ON AIR" : "OFFLINE"}</div>
                </div>
                <div className="mt-3 text-4xl font-semibold tracking-tight tabular-nums">
                  {viewersTarget == null ? "—" : viewers.toLocaleString()}
                </div>
                <div className="mt-2 text-xs text-neutral-400">{isLive ? `Game: ${game}` : "等待下一次開播"}</div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  ...
                  <div className="rounded-xl border border-white/10 bg-black/20 p-3">
                    ...
                    <div className="mt-1 text-sm font-medium">{isLive ? "HYPER MODE" : "IDLE MODE"}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                ...
                <div className="mt-3 line-clamp-2 text-lg font-semibold leading-snug">{title}</div>
                ...
                <div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-3">
                  ...
                  <div className="mt-1 text-sm font-medium">{data?.live?.startedAt ? fmt(data.live.startedAt) : "—"}</div>
                </div>
            ...
            <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {latestEvents.length ? (
                  latestEvents.map((e) => (
                    <div key={e.id} className="rounded-xl border border-white/10 bg-black/20 p-4 hover:bg-black/30 transition-colors">
            ...
            <div className={`rounded-2xl border border-white/10 bg-white/5 p-5 ${liveGlow}`}>
              ...
              <div className="mt-4 space-y-3">
                {(data?.recentVods ?? []).slice(0, 10).map((v) => (
                  ...
                  >
                    <div className="line-clamp-2 font-medium group-hover:text-white text-neutral-200">{v.title}</div>
