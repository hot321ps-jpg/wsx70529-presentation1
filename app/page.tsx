...
export default function Page() {
  ...
  const liveAccent = isLive ? "from-emerald-400/20 via-cyan-400/10 to-fuchsia-500/20" : "from-white/10 via-white/5 to-white/10";
  const liveGlow = isLive ? "shadow-[0_0_60px_rgba(16,185,129,0.18)]" : "shadow-[0_0_60px_rgba(168,85,247,0.10)]";
    ...
    <div className="relative min-h-screen overflow-hidden bg-neutral-950 text-neutral-100">
      ...
      <div className="relative mx-auto max-w-7xl px-5 py-10">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs ${pillTone(status)}`}>
                  <span className={`h-1.5 w-1.5 rounded-full ${status === "live" ? "bg-emerald-300" : status === "degraded" ? "bg-amber-300" : status === "error" ? "bg-red-300" : "bg-neutral-300"}`} />
                  {pillLabel(status)}
                </span>
              </div>
        ...
        <div className="mt-8 text-center text-xs text-neutral-500">
          Tip: 如果畫面一直 DEGRADED/ERROR，先測 <span className="text-neutral-300">/api/warroom-refresh</span> 是否回 <span className="text-neutral-300">{`{ ok: true }`}</span>
        </div>
      </div>

      {/* Tailwind keyframes (inline via arbitrary class) */}
      <style jsx global>{`
        @keyframes marquee {
